...
class _CupertinoTypeAheadPageState extends State<CupertinoTypeAheadPage> {
  ...
  Future<List<String>> _getFoodItems(String pattern) async {
    ...
    if (pattern.isNotEmpty) {
      return Future.delayed(const Duration(seconds: 2), () => foodItems.where((item) => item.toLowerCase().contains(pattern.toLowerCase())).toList());
    } else {
  ...
  Widget _getTypeAhead() {
    ...
    return CupertinoTypeAheadFormField<String>(
      ...
      suggestionsBoxDecoration: const CupertinoSuggestionsBoxDecoration(
        hasScrollbar: true,
      ),
